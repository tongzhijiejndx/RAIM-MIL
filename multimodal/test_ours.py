...
LOCAL_CONFIG["SUMMARY_CSV"] = os.path.join(LOCAL_CONFIG["LOG_ROOT"], "FINAL_MULTIMODAL_SUMMARY_OURS.csv")
...
def build_model_from_ckpt(ckpt):
    ...
    model = MultimodalMILModel(**model_kwargs).to(LOCAL_CONFIG["DEVICE"])
    model.load_state_dict(ckpt["model_state_dict"], strict=True)
    model.eval()
    return model


def predict_one_checkpoint(model, ckpt, pt_path: str):
...
def evaluate_one_experiment(test_pt_files, threshold_map):
    ...
    for current_task in ALL_TASKS:
        ...
        for pt_path in test_pt_files:
            ...
            fold_probs = []
            for model, ckpt in zip(fold_models, fold_ckpts):
                p = predict_one_checkpoint(model, ckpt, pt_path)
                fold_probs.append(p)
...
def run_all_tests():
    ...
    print("========== Runtime Config (Ours Test) ==========")
    print(json.dumps({
