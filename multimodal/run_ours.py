...
def run_all():
    ...
    print("========== Runtime Config (Ours Train) ==========")
    ...
    for current_task in ALL_TASKS:
        ...
        for fold, (train_idx, val_idx) in fold_pbar:
            ...
            model = build_model(clin_dim=clin_dim)
            ...
            for epoch in epoch_pbar:
                train_loss = train_one_epoch_multimodal(
                    ...
                    use_attn_prior=policy["use_attn_prior"],
                    alpha_attn=policy["alpha_attn"],
                    attn_warmup_epochs=policy["attn_warmup_epochs"],
                    attn_prior_eps=policy["attn_prior_eps"],
                    attn_supervision_mode=policy["attn_supervision_mode"],
                    use_neg_suppress=policy["use_neg_suppress"],
                    beta_neg=policy["beta_neg"],
                    neg_warmup_epochs=policy["neg_warmup_epochs"],
                    neg_margin=policy["neg_margin"],
                    neg_supervision_mode=policy["neg_supervision_mode"],
                    neg_topk_ratio=policy["neg_topk_ratio"],
                    epoch_idx=epoch,
                )

                val_out = evaluate_one_epoch_multimodal(
                    model=model,
                    loader=val_loader,
                    criterion_bag=criterion_bag,
                    current_task=current_task,
            ...
            with torch.no_grad():
                for batch in val_loader:
                    ...
                    clin_feats = batch["clin_feats"].to(LOCAL_CONFIG["DEVICE"])
                    labels = batch["label"]
            ...
            fold_auc = 0.5
            if len(np.unique(np.asarray(fold_val_true).astype(int))) > 1:
                from sklearn.metrics import roc_auc_score
                fold_auc = roc_auc_score(fold_val_true, fold_val_probs)
    ...
    summary_csv = os.path.join(LOCAL_CONFIG["LOG_ROOT"], "FINAL_MULTIMODAL_SUMMARY_OURS.csv")
