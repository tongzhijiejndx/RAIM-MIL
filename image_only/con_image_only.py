...
CONFIG = {
    "PT_ROOT": os.path.join(ROOT_DIR, "data", "features_test_roi"),
    "MAX_PATCHES": 4000,
    ...
    "EXPERIMENTS": [
        {
            "name": "ABMIL",
            "log_root": os.path.join(CUR_DIR, "logs_image_only"),
            "enabled": True,
        },
        {
            "name": "CLAM",
...
def load_ckpt_files(log_root: str, exp_name: str, task_name: str) -> List[str]:
    # 支持两种目录结构
    # 1) logs_image_only/<Task>/<Experiment>/fold*_best.pth
    # 2) logs_image_only/<Experiment>/<Task>/fold*_best.pth
    candidates = [
        os.path.join(log_root, task_name, exp_name),
        os.path.join(log_root, exp_name, task_name),
    ]
    ...
    raise FileNotFoundError(
        f"No fold*_best.pth found for exp={exp_name}, task={task_name} under {log_root}"
    )


def build_model_from_ckpt(ckpt: Dict[str, Any]):
    if "model_name" not in ckpt:
        raise KeyError("Checkpoint missing 'model_name'")
    if "model_kwargs" not in ckpt:
        raise KeyError("Checkpoint missing 'model_kwargs'")
...
@torch.no_grad()
def forward_one_pt_with_ckpt(pt_path: str, ckpt_path: str):
...
@torch.no_grad()
def forward_one_pt_ensemble(pt_path: str, ckpt_paths: List[str]):
    outs = [forward_one_pt_with_ckpt(pt_path, ck) for ck in ckpt_paths]
...
def run_one_experiment(exp_name: str, log_root: str):
    print("=" * 100)
    print(f"[ROI-CONSISTENCY] {exp_name}")
    print("=" * 100)
    print(f"[INFO] PT_ROOT  : {CONFIG['PT_ROOT']}")
    print(f"[INFO] LOG_ROOT : {log_root}")

    pt_files = load_pt_files(CONFIG["PT_ROOT"])
    ...
    for task_name in CONFIG["TASKS"]:
        try:
            ckpt_paths = load_ckpt_files(log_root, exp_name, task_name)
        ...
        for idx, pt_path in enumerate(pt_files, start=1):
            ...
            try:
                out = forward_one_pt_ensemble(pt_path, ckpt_paths)
        ...
        case_csv = os.path.join(save_dir, f"{task_name}_roi_case_metrics.csv")
...
def merge_all_experiments():
    ...
    out_csv = os.path.join(CONFIG["SAVE_DIR"], "all_experiments_roi_consistency_summary.csv")
...
def main():
    ...
    for item in CONFIG["EXPERIMENTS"]:
        if not item["enabled"]:
            continue
        try:
            run_one_experiment(item["name"], item["log_root"])
        except Exception as e:
    ...
    print("=" * 100)
    print("Attention-ROI consistency analysis for image-only models completed.")
    print("=" * 100)
